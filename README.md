# 【ICLR 2026】MergOPT: Guiding Feasible Region for Weight Shifts via Merging-Aware Optimizer 
...
During expert fine-tuning, MergOPT injects merge-like weight offsets into each update step. This trains each expert to be more robust to future parameter merging. Most merging methods only handle conflicts at merge time. MergOPT improves mergeability earlier, at fine-tuning time, so the final merged model tends to be more stable and performs better across tasks.
